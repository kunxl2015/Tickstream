...
int main()
{
    ...
    // Priority queue to pick the record with the smallest timestamp each time.
    // We need a min-heap, but C++ priority_queue is max-heap by default, so we invert the comparison.
    priority_queue<DataPoint, vector<DataPoint>, greater<DataPoint>> pq;

    // Helper function to read the next record from a given input file.
